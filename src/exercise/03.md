...
JSX allows for syntatic sugar that allows us to create React elements using
HTML-like syntax. Tools like Babel can be used to compile JSX to Javascript,
using the React APIs instead of the HTML-like syntax.

To interpolate values in JSX, curly braces can be used surrounding Javascript
expressions.

To interpolate an entire `props` object onto a JSX element, use the spread
operator, i.e. `{...props}`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=yost90%40gmail.com).
