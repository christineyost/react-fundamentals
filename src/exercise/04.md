...
- Custom components = functions that return a React element
- Custom components can be used as the first argument (type) to
  React.createElement()
- If the custom component starts with a capital letter, it can be used in JSX
  (for example `<Message/>`)
- React fragments - denoted by empty brackets `<>`, `</>`. These are useful
  because React elements need to have a single parent in order to be rendered.
  Fragments allow for an empty parent in case one wants to render side-by-side
  elements

### Proptypes

- Runtime validation of custom component properties
- These do not run in production due to performance
- The React team maintains a prop-types package with many validators for
  different types
- Proptypes are not useful with Typescript or other typed languages, since those
  already have type validation
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=04%3A%20Creating%20custom%20components&em=yost90%40gmail.com).
