...
- In submit handlers we can use `event.preventDefault()` to avoid a browser
  refresh which is default submit behavior
- We can get elements from events by id using `event.target.elements.<id>` (here
  target is the form that submit was called for)
- We can get the value from the element by calling
  `event.target.elements.<id>.value`
- The `htmlFor` prop maps a label to another html element. When this is a label
  mapped to an input, this makes it so clicking on the label brings focus to the
  input. It also helps screenreaders
- We can also use React hook `useRef` to get the current state of an html
  element. See `06.js`
- The `useState` hook can be used to monitor state. See `06.js` to see it
  monitoring error state of an input field
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=06%3A%20Forms&em=yost90%40gmail.com).
